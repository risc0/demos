...
export function ProveButton({ address }: { address: string }) {
	...
	const [_snarkResults, setSnarkResults] = useLocalStorage<any | undefined>(
		"snark-results",
		undefined,
	);
	const [googleUserInfos] = useLocalStorage("google-infos", undefined);
	const [googleUserToken] = useLocalStorage<string | undefined>(
		"google-token",
		undefined,
	);
			...
			{isPending ? (
				...
				<>
					<p className="mb-3 break-words text-xs">
						...
						<strong title={address}>{address}</strong> owns the following social
						account:
					</p>

					{googleUserInfos && (
						<UserInfos type="google" userInfos={googleUserInfos} />
					)}
				</>
			)}
			...
			<div className="mt-6 w-full">
				...
				>
					...
					<img
						...
						width={58}
						height={16}
						src="https://zkauth.vercel.app/images/bonsai-logo-light.svg"
						alt="bonsai logo"
					/>
				</Button>
						...
						<AlertTitle>
							...
							<span
								className={cn(
									...
									starkPollingResults.at(-1)?.status === "FAILED" &&
				...
				{snarkPollingResults && (
					<Alert className="border-none px-0 pb-0">
						<AlertTitle>
							...
							<span
								className={cn(
									snarkPollingResults.status === "SUCCEEDED" &&
										"font-bold text-green-600 dark:text-green-500",
									snarkPollingResults.status === "FAILED" &&
										"font-bold text-red-600 dark:text-red-500",
								)}
							>
						...
						<AlertDescription className="rounded border bg-neutral-50 font-mono dark:bg-neutral-900">
							<div className="flex flex-row items-center justify-between gap-2 px-3 py-2 text-xs">
								...
								<Loader2Icon className="size-3.5 animate-spin text-border" />
							</div>
						</AlertDescription>
					</Alert>
				)}
