...
contract ZRPTest is Test {
    ...
    // function bob_deposit(bytes memory ident, uint256 amount, uint256 salt) internal {
    //     vm.prank(bob);
    //     zrp.deposit{value: amount}(ident, address(0), amount, salt);
    // }
    //
    // function bob_deposit_token(bytes memory ident, uint256 amount, uint256 salt) internal {
    //     vm.prank(bob);
    ...
    //     assertEq(zrp.claimBalance(claimId), 2 ether, "claim should have 2 ether");
    // }
    //
    // // Valid claim
    function test_Mint() public {
        // bob_depogcsit(ALICE_IDENT, 1 ether, block.number);
        // carl_deposit(ALICE_IDENT, 1 ether);
        ...
        vm.prank(alice);
        zrp.mint(abi.encode(proof), "TEST");
    ...
    //     // malicious proof
    //     Types.Proof memory proof =
    //         Types.Proof({seal: "0x6666", postStateDigest: bytes32(0x0), journal: abi.encode(alice, ALICE_IDENT)});
    //
    //     vm.prank(malice);
    ...
    //
    //     assertEq(bob.balance, 5 ether, "bob should have 5 ether after clawback");
    //
    //     Types.Proof memory proof =
    //         Types.Proof({seal: "0x4141", postStateDigest: bytes32(0x0), journal: abi.encode(alice, ALICE_IDENT)});
    //
    //     // alice claims
    //     vm.prank(alice);
    //     zrp.claim(abi.encode(proof), address(0));
    ...
    //     zrp.claim(abi.encode(proof), address(0));
    //
    //     assertEq(alice.balance, 0, "alice should have 0 ether after both participants clawback");
    // }
    //
    ...
    //
    //     assertEq(
    //         zrp.depositBalance(zrp.getDepositId(bob, ALICE_IDENT, address(0), block.number - 1)),
    //         0,
    ...
    //     vm.prank(alice);
    //     zrp.claim(abi.encode(proof), address(0));
    //     assertEq(alice.balance, 1 ether, "alice should have 1 ether after both participants did their thing");
    //
    ...
    //     assertEq(alice.balance, 4 ether, "alice should have 4 ether after both participants did their thing");
    // }
    //
    // function test_ManyDeposits() public {
    //     for (uint256 i = 0; i < whale.balance; ++i) {
    //         vm.prank(whale);
    //         zrp.deposit{value: 1 ether}(ALICE_IDENT, address(0), 1 ether, block.number);
    //         rollForward();
    //     }
    //
    //     vm.prank(alice);
    ...
    //     assertEq(address(zrp).balance, 0.01 ether, "contract should have 0.01 ether");
    // }
    //
    ...
    //
    //     assertEq(address(zrp).balance, 0, "contract should have 0 ether after withdraw");
    // }
    //
    ...
    //     assertEq(dai.balanceOf(bob), 1_000_000e18, "bob should have 1_000_000e18 dai");
    //     bob_deposit_token(ALICE_IDENT, 1_000e18, block.number);
    //     assertEq(dai.balanceOf(bob), 999_000e18, "bob should have 999_000e18 dai after deposit");
    //     assertEq(dai.balanceOf(address(zrp)), 1_000e18, "contract should have 1_000e18 dai after deposit");
    // }
    //
    // function test_ClaimToken() public {
    //     bob_deposit_token(ALICE_IDENT, 1_000e18, block.number);
    //
    //     assertEq(dai.balanceOf(alice), 0, "alice should have 0 dai before claim");
    //     vm.prank(alice);
    //     zrp.claim(
    //         abi.encode(
    //             Types.Proof({seal: "0x4141", postStateDigest: bytes32(0x0), journal: abi.encode(alice, ALICE_IDENT)})
    ...
    //     );
    //
    //     assertEq(dai.balanceOf(alice), 1_000e18, "alice should have 1_000e18 dai after claim");
    // }
}
