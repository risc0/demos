...
use revm::{
    primitives::{Bytes, TxKind, Address, U256, ExecutionResult, Output},
    InMemoryDB, Evm,
};
...
fn main() {
    ...
    evm.tx_mut().data = Bytes::from(config.bytecode.clone());
    ...
    // Call the function
    evm.tx_mut().transact_to = TxKind::Call(contract_address);
    evm.tx_mut().data = Bytes::from(calldata);
    evm.tx_mut().nonce = Some(1);
    let call_result = evm.transact_commit().unwrap();
