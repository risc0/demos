...
mod tests {
    ...
    fn test_base58_successes() {
        for (encoded, hash) in [
            ...
            (
                "CjNSmWXTWhC3EhRVtqLhRmWMTkRbU96wUACqxMtV1uGf",
                CryptoHash::hash_bytes(&[0, 1, 2]),
            ),
        ] {
    ...
    fn test_from_str_failures() {
        fn test(input: &str, want_err: &str) {
            match CryptoHash::from_str(input) {
                ...
                Err(err) => {
                    assert!(
                        err.to_string().starts_with(want_err),
                        "input: `{input}`; err: {err}"
                    )
                }
            }
        ...
        test(
            "foo-bar-baz",
            "provided string contained invalid character '-' at byte 3",
        );
